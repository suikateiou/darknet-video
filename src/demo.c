...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes, int avgframes,
    ...
    int benchmark, int benchmark_layers, char *outfile)
{
    ...
    if (out_filename && !flag_exit)
    {
        ...
        output_video_writer =
            create_video_writer(out_filename, 'X', 'V', 'I', 'D', src_fps, get_width_mat(det_img), get_height_mat(det_img), 1);
    ...
    char* folder_name = strtok (pre, ".");
    char dir[1024];
    strcpy(dir, "/home/zju/czh/carla_dataset/detect_pics/");
    strcat(dir, folder_name);
    mkdir(dir, 0777);

    printf ("save results images into folder %s\n", dir);
    ...
    while(1){
        ++count;
        {
            ...
            if (json_file) {
                ...
                ++json_image_id;
                //origin
                // json_buf = detection_to_json(dets, nboxes, l.classes, names, json_image_id, NULL);
                
                //changeto
                json_buf = detection_to_json(dets, nboxes, l.classes, names, frame_id, NULL);

                fwrite(json_buf, sizeof(char), strlen(json_buf), json_file);
                free(json_buf);
                // printf("json img id: %d", json_image_id);
            }

            
            // printf("frameid: %d", frame_id);
            if (demo_json_port > 0) {
                int timeout = 400000;
            ...
            if (!benchmark && !dontdraw_bbox) draw_detections_cv_v3_yyy(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, frame_id, folder_name);
    ...
    // int inft = std::chrono::duration_cast<std::chrono::seconds>(end - start).count();
    // wyy_fps = count / inft;

    double end = get_time_point();
    float wyy_fps = 1000000. / (end - start) * count;
    printf("=============== AVG FPS: %f =============== \n", wyy_fps);
