...
extern "C" void write_frame_cv(write_cv *output_video_writer, mat_cv *mat)
{
    try {
        ...
        //yyy
        // cv::Mat img_src = *(cv::Mat*)mat;
        // ycount++;
        // char buff[256];
        // sprintf(buff, "%s_%08d.jpg", "yyy", ycount);
        //     cv::Mat img_rgb;
        //     if (img_src.channels() >= 3) cv::cvtColor(img_src, img_rgb, cv::COLOR_RGB2BGR);
        //     stbi_write_jpg(buff, img_rgb.cols, img_rgb.rows, 3, (char *)img_rgb.data, 80);
        // save_mat_jpg(*(cv::Mat*)mat, buff);
    }
    catch (...) {
    ...
    try {
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                ...
                sprintf(image_name, "result_img/img_%07d_%07d_%03d_%s.jpg", frame_id, img_id, class_id, names[class_id]);
                printf("%s\n", image_name);
...
extern "C" void draw_detections_cv_v3_yyy(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, long long frame_id, char* folder_name)
{
    printf("draw detections cv v3 yyy\n");
    try {
        cv::Mat *show_img = (cv::Mat*)mat;
        int i, j;
        if (!show_img) return;
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                ...
                std::string class_name(names[class_id]);
                ...
                if (class_name == "car" || class_name == "truck" || class_name == "bus"){
                    ...
                    sprintf(image_name, "/home/zju/czh/carla_dataset/detect_pics/%s/%s_%04d_%07d.jpg", folder_name, folder_name, frame_id, img_id);
                    printf("\n%s\n", image_name);
                    CvRect rect = cvRect(pt1.x, pt1.y, pt2.x - pt1.x, pt2.y - pt1.y);
                    cvSetImageROI(copy_img, rect);
                    // try{
                    ...
                    // }
                    // catch (...){
                    //     printf("Fail to save the image\n");
                    // }

                    cv::Mat Img;
                    Img = cv::cvarrToMat(copy_img);
        ...
        cerr << "OpenCV exception: draw_detections_cv_v3_yyy() \n";
    }
}
// ----------------------------------------
